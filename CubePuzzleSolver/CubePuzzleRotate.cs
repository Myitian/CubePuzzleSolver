...
namespace Myitian.CubePuzzleSolver
{
    ...
    public class CubePuzzleRotate
    {
        ...
        public CubePuzzleRotate(int[] interactiveCube, int[][] map, int[] noninteractiveCube = null)
        {
        ...
        private void PrivateInteract(int n)
        {
            if (n >= 0) //正数和零操作可交互机关立方
            {
                ...
                InteractiveCube_Current[n] &= 3; //相当于 %= 4，但位运算快一点
            }
            else //负数操作不可交互机关立方（我也不知道有没有这种机关立方，总之先加着）
            {
                NoninteractiveCube_Current[-1 - n]++;
                NoninteractiveCube_Current[-1 - n] &= 3; //相当于 %= 4，但位运算快一点
            }
        }
